...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3, init_channels=3):
        ...
        if self.hybrid:
            self.hybrid_model = ResNetV2(
                block_units=config.resnet.num_layers, width_factor=config.resnet.width_factor, init_channels=init_channels)
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # print('(En) hidden_states.shape: ', hidden_states.shape)
        encoded = self.encoder_norm(hidden_states)
        # print('(En) encoded.shape: ', encoded.shape)
        return encoded, attn_weights


class Transformer(nn.Module):
    def __init__(self, config, img_size, vis, init_channels=3):
        super(Transformer, self).__init__()
        self.embeddings = Embeddings(
            config, img_size=img_size, init_channels=init_channels)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.transformer = Transformer(config, img_size, vis, init_channels=3)
        ...
        self.transformer2 = Transformer(config, img_size, vis, init_channels=9)
    ...
    def forward(self, x, coord_batch=None):
        sigmoid = nn.Sigmoid()
        y = x
        if x.size()[1] == 1:
            x = x.repeat(1, 3, 1, 1)
        ...
        logits1 = self.segmentation_head(x)
        y = torch.cat([logits1, y], dim=1)

        y, attn_weights, features = self.transformer2(y)  # (B, n_patch, hidden)
        y = self.decoder2(y, features)
        logits2 = self.segmentation_head2(y)
